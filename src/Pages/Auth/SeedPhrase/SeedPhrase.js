...
function SeedPhrase() {
    ...
    async function loginUser() {
        ...
        let accountCreated_Balance = await web3.eth.getBalance(wallet_account_address); //pourquoi 0 ? wallet.address
        let accountCreated_Balance_Nub = Number(accountCreated_Balance);

        const privatekey = wallet.privateKey;
        // Encrypt with crypto js, le mdp initial est utilisé pour crypter les privatekey et seedphrase.
        ...
        passwordGlobale.push(userPassword)

        let userData = {
            selectedAddressId: wallet.address,
            accountAddresses: [
                {
                addressName: "Account 1",
                address: wallet.address,
                balance: accountCreated_Balance_Nub,
                privatekey:  wallet.privateKey
                },
            ],
            //Le mot de pass lui mm est hashé, donc on ne peut plus jamais avoir sa valeur initiale.
